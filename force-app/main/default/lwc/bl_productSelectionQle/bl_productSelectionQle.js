...
export default class Bl_productSelectionQle extends NavigationMixin(LightningElement) {
    ...
    saveBeforeConfigured(event){
        ...
        this.savePSValues = true;
        this.quotesAdded = []; 
        let mergeAllQuoteline = []; 
        mergeAllQuoteline.push(this.girdDataAcaTabAdd, this.girdDataConnTabAdd, this.girdDataFocTabAdd,
            this.girdDataCableTabAdd, this.girdDataTandITabAdd);

        if(mergeAllQuoteline.length > 0){
            //console.log('ACA LIST');
            let list1 = JSON.parse(JSON.stringify(mergeAllQuoteline)); 
